...
Vagrant.configure("2") do |config|
  config.vm.define "pg1" do |primary|
    ...
    primary.vm.network "private_network", ip: "192.168.192.11"
    primary.vm.provider "vmware_desktop" do |vmp|
      vmp.memory = "1024"
      vmp.cpus = "1"
    end
  ...
  config.vm.define "pem1", primary: true  do |witness_pem|
    ...
    witness_pem.vm.network "private_network", ip: "192.168.192.10"
    witness_pem.vm.network "forwarded_port", guest: 8443, host: 8400
    witness_pem.vm.provider "vmware_desktop" do |vmw|
      vmw.memory = "1024"
      vmw.cpus = "1"
    end
  ...
  (2..3).each do |i|
     config.vm.define "pg#{i}" do |standby|
       ...
       standby.vm.network "private_network", ip: "192.168.192.1#{i}"
       standby.vm.provider "vmware_desktop" do |v|
         v.memory = "1024"
         v.cpus = "1"
       end
  ...
  config.vm.define "backup1" do |backup|
    ...
    backup.vm.network "private_network", ip: "192.168.192.14"
    backup.vm.provider "vmware_desktop" do |vms2|
      vms2.memory = "512"
      vms2.cpus = "1"
    end
  ...
  (1..3).each do |i|
     config.vm.define "pooler#{i}" do |pooler|
      ...
      pooler.vm.network "private_network", ip: "192.168.192.2#{i}"
      pooler.vm.provider "vmware_desktop" do |v|
        v.memory = "512"
        v.cpus = "1"
      end
