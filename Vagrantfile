...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "pg1" do |primary|
    ...
    primary.vm.provision "shell", inline: <<-SHELL
      sh /vagrant_keys/generate_public_key.sh
      #sh /vagrant_keys/copy_keys.sh
    SHELL
  end
  ...
  config.vm.define "pem1", primary: true  do |witness_pem|
    ...
    #witness_pem.vm.network "public_network"    
    ...
    witness_pem.vm.provision "shell", inline: <<-SHELL
      sh /vagrant_keys/copy_keys.sh
    SHELL
  end
  ...
  (2..3).each do |i|
     config.vm.define "pg#{i}" do |standby|
       ...
       standby.vm.provision "shell", inline: <<-SHELL
         sh /vagrant_keys/copy_keys.sh
       SHELL
     end
  end
  ...
  (1..3).each do |i|
     config.vm.define "pooler#{i}" do |pooler|
      ...
      pooler.vm.provision "shell", inline: <<-SHELL
        sh /vagrant_keys/copy_keys.sh
      SHELL
    end
  end
